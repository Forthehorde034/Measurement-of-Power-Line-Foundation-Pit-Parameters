...
import sys
import threading
# 防止 --windowed 模式下因 stderr 无效导致 C++ 库崩溃
...
from algo import pit_pipeline_analysis as analysis
...
class AppWindow:
    MENU_OPEN = 11
    MENU_CALC_PARAMETERS = 12
    MENU_EXPORT_POINT_CLOUD = 13
    MENU_EXPORT = 14
    MENU_QUIT = 15
    ...
    def __init__(self, title, width, height):
        ...
        # ---- 设置面板 ----
        # 与其使用像素指定尺寸（在不同显示器上大小可能不同，特别是在 macOS 上，
        # 其 DPI 通常为 220），不如使用 em-size。这样尺寸将与字体大小成比例，
        # 从而在不同平台上创建视觉上一致的尺寸。
        em = w.theme.font_size
        separation_height = int(round(0.5 * em))

        # 控件通过布局进行排列：gui.Horiz、gui.Vert、
        # gui.CollapsableVert 和 gui.VGrid。通过嵌套布局我们可以
        # 实现复杂的设计。通常我们使用垂直布局作为最顶层的控件，
        # 因为控件通常从上到下组织。在其中，我们通常为每一行
        # 使用一系列水平布局。所有布局都有一个 spacing 参数，
        # 表示控件中各项之间的间距，以及一个 margins 参数，
        # 指定左、上、右、下边距的间距。（这类似于 CSS 中的 'padding' 属性。）
        self._settings_panel = gui.Vert(
            0, gui.Margins(0.25 * em, 0.25 * em, 0.25 * em, 0.25 * em))

        # 在设置面板中添加结果显示区域
        calculate_results = gui.CollapsableVert("Calculate", 0,
                                                gui.Margins(em, 0, 0, 0))

        result_panel = gui.VGrid(2, 0.25 * em)
        result_panel.add_child(gui.Label("Depth"))
        self._depth_result = gui.TextEdit()
        result_panel.add_child(self._depth_result)

        result_panel.add_child(gui.Label("Diameter"))
        self._diameter_result = gui.TextEdit()
        result_panel.add_child(self._diameter_result)

        result_panel.add_child(gui.Label("Verticality"))
        self._verticality_result = gui.TextEdit()
        result_panel.add_child(self._verticality_result)

        calculate_results.add_child(result_panel)

        self._settings_panel.add_fixed(separation_height)
        self._settings_panel.add_child(calculate_results)


        # 创建一个可折叠的垂直控件，打开时占用足够的垂直空间来容纳所有子控件，
        # 关闭时只占用文本所需的空间。这对于属性页面很有用，
        # 用户可以隐藏他们很少使用的属性集。
        ...
        # 我们想要两行按钮，所以创建两个水平布局。我们还希望按钮居中，
        # 可以通过在第一个和最后一个位置放置伸展项来实现。
        # 伸展项占用尽可能多的空间，由于有两个伸展项，
        # 它们将各占一半的额外空间，从而使按钮居中。
        ...
        '''
        通常我们的用户界面可以是所有一个布局的子控件（通常是垂直布局），
        然后这是窗口的唯一子控件。在我们的情况下，我们希望场景占据所有空间，
        而设置面板在其上方。我们可以通过提供 on_layout 回调来实现这种自定义布局。
        on_layout 回调应该正确设置每个子控件的框架（位置+大小）。
        回调完成后，窗口将布局孙控件。
        '''
        ...
        if gui.Application.instance.menubar is None:
            ...
            file_menu.add_item("Calc Parameter", AppWindow.MENU_CALC_PARAMETERS)
            file_menu.add_item("Export PCD File", AppWindow.MENU_EXPORT_POINT_CLOUD)
        ...
        w.set_on_menu_item_activated(AppWindow.MENU_CALC_PARAMETERS,
                                     self._on_menu_calc_parameters)
        w.set_on_menu_item_activated(AppWindow.MENU_EXPORT_POINT_CLOUD,
                                     self._on_menu_export_point_cloud)
    ...
    def _on_menu_calc_parameters(self):

        """
        计算参数 - 使用线程处理
        """
        if self._geometry is None:
            self.window.show_message_box("Error", "No point cloud loaded")
            return

        # 显示计算中提示
        self._update_result_labels("Calculating...", "Calculating...", "Calculating...")
        # 在后台线程中执行计算
        threading.Thread(target=self._calculate_parameters_thread, daemon=True).start()


    def _calculate_parameters_thread(self):
        """
        在后台线程中执行参数计算
        """
        try:
            ...
            metrics, cropped_pcd = analysis.calculate_pit_pipeline_pcd_data(self._geometry.to_legacy(), crop_params, analysis_params)

            # 在主线程中更新UI和显示裁剪后的点云
            def update_ui():
                if metrics:
                    MAX_VERTICALITY_DEG = np.degrees(np.arctan(0.01))
                    verticality_status = "Pass"
                    if metrics.verticality_deg > MAX_VERTICALITY_DEG:
                        verticality_status = "Fail"

                    logger.info("\n================== 最终计算结果 ==================")
                    logger.info(f"**总 坑 深 (Depth): {metrics.depth:.3f} 米**")
                    logger.info(f"**基坑 平均直径 (Avg. D): {metrics.avg_diameter:.3f} 米**")
                    logger.info("--- 垂直度校验 ---")
                    logger.info(f"**基坑 垂直度误差: {metrics.verticality_deg:.3f} 度 (0° 为完美垂直)**")
                    logger.info(f"**基坑合格情况: {verticality_status}**")

                    self._update_result_labels(
                        f"{metrics.depth:.3f} m",
                        f" {metrics.avg_diameter:.3f} m",
                        f" {metrics.verticality_deg:.3f}° {verticality_status}"
                    )
                    
                    # 只有在计算成功时才显示裁剪后的点云
                    if cropped_pcd is not None:
                        # 将 legacy 点云转换为 tensor 点云
                        cropped_tensor_pcd = o3d.t.geometry.PointCloud.from_legacy(cropped_pcd)
                        # 清空场景并添加裁剪后的点云
                        self._scene.scene.clear_geometry()
                        self._geometry = cropped_tensor_pcd
                        self._scene.scene.add_geometry("__model__", cropped_tensor_pcd, self.settings.material)
                        # # 更新相机视角
            ...
            def show_error():
                self._update_result_labels("", "", "")
                self.window.show_message_box("Error", f"Calculation failed: {str(e)}")

            gui.Application.instance.post_to_main_thread(self.window, show_error)

    def _update_result_labels(self, depth_text, diameter_text, verticality_text):
        """
        更新结果显示标签
        """
        self._depth_result.text_value = depth_text
        self._diameter_result.text_value = diameter_text
        self._verticality_result.text_value = verticality_text

    def _on_menu_export_point_cloud(self):
        # 创建文件保存对话框
        dlg = gui.FileDialog(gui.FileDialog.SAVE, "export pcd file",
                             self.window.theme)
        dlg.add_filter(".pcd", "PCD File (.pcd)")
        dlg.add_filter(".ply", "PLY File (.ply)")
        dlg.set_on_cancel(self._on_file_dialog_cancel)
        dlg.set_on_done(self._on_export_point_cloud_done)
        self.window.show_dialog(dlg)

    def _on_export_point_cloud_done(self, filename):
        self.window.close_dialog()
        try:
            # 获取当前场景中的点云几何体
            geometry = self._geometry.to_legacy()
            if geometry is not None:
                # 根据文件扩展名选择合适的写入方法
                if filename.endswith(".ply"):
                    o3d.io.write_point_cloud(filename, geometry)
                elif filename.endswith(".pcd"):
                    o3d.io.write_point_cloud(filename, geometry)
                # 可以添加更多格式支持
        except Exception as e:
            logger.error(f"导出点云失败: {e}")

    def _on_menu_export(self):
        dlg = gui.FileDialog(gui.FileDialog.SAVE, "Choose file to save",
                             self.window.theme)
        dlg.add_filter(".png", "PNG files (.png)")
        dlg.add_filter(".jpg", "JPG files (.jpg)")
        dlg.set_on_cancel(self._on_file_dialog_cancel)
    ...
    def _on_menu_about(self):
        ...
        dlg_layout.add_child(gui.Label("PcdViewer"))
        dlg_layout.add_child(gui.Label("Version:1.0.0"))
    ...
    def load(self, path):
        # 显示加载提示
        threading.Thread(target=self._load_point_cloud, args=(path,), daemon=True).start()
    ...
    def _load_point_cloud(self, path):
        ...
        def on_done():
            ...
            if mesh is None:
                logger.info( path, "appears to be a point cloud")
                cloud = None
                try:
                ...
                if cloud is not None:
                    logger.info("Successfully read", path)

                    # ====== 关键：安全判断 intensity 是否存在 ======

                    if "intensity" in cloud.point:
                        logger.info("Using intensity for coloring")
                        # 3. 将 intensity 转为 colors
                        intensity = cloud.point.intensity
                        try:
                            # 支持标量或单通道向量（如 shape=(N,) 或 (N,1)）
                            if intensity.ndim > 1 and intensity.shape[1] == 1:
                                intensity = intensity.reshape(-1)

                            intensity_np = intensity.numpy()
                            intensity_norm = (intensity_np - intensity_np.min()) / (
                                        intensity_np.max() - intensity_np.min() + 1e-8)
                            colors = np.tile(intensity_norm[:, None], (1, 3))  # 灰度

                            cloud.point.colors = o3d.core.Tensor(colors, dtype=o3d.core.float32)
                        except Exception as e:
                            logger.error("Failed to convert intensity to colors",e)

                    geometry = cloud
                else:
                    logger.warn("Failed to read points", path)
        ...
        gui.Application.instance.post_to_main_thread(self.window, on_done)
...
def main():
    ...
    w = AppWindow("PcdViewer",1920, 1080)
